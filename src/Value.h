...
#define SRCLANG_VALUE_IS_CLOSURE(val)                                          \
    (SRCLANG_VALUE_IS_OBJECT((val)) &&                                         \
            SRCLANG_VALUE_AS_OBJECT((val))->type == ValueType::Closure)
#define SRCLANG_VALUE_IS_ERROR(val)                                            \
    (SRCLANG_VALUE_IS_OBJECT((val)) &&                                         \
...
#define SRCLANG_VALUE_AS_STRING(val)                                           \
    ((wchar_t*)SRCLANG_VALUE_AS_OBJECT(val)->pointer)
#define SRCLANG_VALUE_AS_LIST(val)                                             \
    ((SrcLangList*)SRCLANG_VALUE_AS_OBJECT(val)->pointer)
...
#define SRCLANG_VALUE_POINTER(ptr)                                             \
    SRCLANG_VALUE_HEAP_OBJECT(ValueType::Pointer, ptr)
...
#define SRCLANG_VALUE_DEBUG(val)                                               \
    (SRCLANG_VALUE_GET_STRING(val) + L":" +                                    \
            s2ws(SRCLANG_VALUE_TYPE_ID[(int)SRCLANG_VALUE_GET_TYPE(val)]))

#define SRCLANG_MODULE_INIT_FUN srclang_module_init
#define SRCLANG_MODULE_INIT                                                    \
    extern "C" void SRCLANG_MODULE_INIT_FUN(                                   \
            SrcLang::SrcLangMap* map, SrcLang::Interpreter* interpreter)
#define SRCLANG_MODULE_DEFINE(id, value) map->insert({s2ws(#id), value})

#define SRCLANG_MODULE_DEFINE_CONST(id)                                        \
    SRCLANG_MODULE_DEFINE(id, SRCLANG_VALUE_NUMBER(id))

#define SRCLANG_MODULE_FUNC(id)                                                \
    static SrcLang::Value module_fun_##id(                                     \
            std::vector<SrcLang::Value> const& args,                           \
            SrcLang::Interpreter* interpreter)
...
#define SRCLANG_CHECK_ARGS_ATLEAST(count)                                      \
    if (args.size() < count)                                                   \
        throw std::runtime_error("Expected atleast '" +                        \
...
#define SRCLANG_CHECK_ARGS_TYPE(pos, ty)                                       \
    if (SRCLANG_VALUE_GET_TYPE(args[pos]) != ty)                               \
        throw std::runtime_error(                                              \
                "Expected '" + std::to_string(pos) + "' to be '" +             \
                SRCLANG_VALUE_TYPE_ID[int(ty)] + "' but " +                    \
                ws2s(SRCLANG_VALUE_DEBUG(args[pos])) + "' provided");

typedef std::vector<Value> SrcLangList;
typedef std::map<std::wstring, Value> SrcLangMap;
...
std::wstring SRCLANG_VALUE_GET_STRING(Value val);
