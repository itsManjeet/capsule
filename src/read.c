...
static CapsuleError parse_simple(const char* start, const char* end, Capsule* result) {
...
static CapsuleError read_string(const char* start, const char* end, Capsule* result) {
...
static CapsuleError read_list(char list_start, char list_end, const char* start, const char** end, Capsule* result) {
    Capsule p;
    ...
    *end = start;
    ...
    for (;;) {
        ...
        if (token[0] == '.' && *end - token == 1) {
            ...
            err = lex(*end, &token, end);
            if (!err && token[0] != list_end)
                err = CAPSULE_ERROR_SYNTAX;
...
CapsuleError read(const char* input, const char** end, Capsule* result) {
    ...
    if (token[0] == '(') {
        ...
        *result = CAPSULE_CONS(CAPSULE_SYMBOL("QUOTE"), CAPSULE_CONS(Capsule_nil, Capsule_nil));
        return read(*end, end, &CAPSULE_CAR(CAPSULE_CDR(*result)));
    } else if (token[0] == '`') {
        *result = CAPSULE_CONS(CAPSULE_SYMBOL("QUASIQUOTE"), CAPSULE_CONS(Capsule_nil, Capsule_nil));
        return read(*end, end, &CAPSULE_CAR(CAPSULE_CDR(*result)));
    } else if (token[0] == ',') {
        *result =
            CAPSULE_CONS(CAPSULE_SYMBOL(token[1] == '@' ? "UNQUOTE-SPLICING" : "UNQUOTE"), CAPSULE_CONS(Capsule_nil, Capsule_nil));
        return read(*end, end, &CAPSULE_CAR(CAPSULE_CDR(*result)));
    } else if (token[0] == '"') {
